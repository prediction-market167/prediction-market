  ...
  return (
    <div className="animate-slide-up">
      ...
      <div className="flex items-center gap-3 mb-5">
        {(() => {
          const t = TIERS.find((t) => t.id === activeTier)!
          return (
            <>
              <div className={`w-1 h-5 rounded-full bg-gradient-to-b ${t.gradient}`} />
              <h2 className="text-lg font-black text-ink-100">{t.name} Markets</h2>
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${t.accentBg} ${t.accentColor} border ${t.accentBorder}`}>
                {liveCount} live
              </span>
            </>
          )
      ...
      {isLoading ? (
        ...
        <div className="text-center py-24 rounded-3xl bg-surface-800/50 border border-surface-700">
          ...
          <p className="text-ink-400 text-lg font-bold">No markets yet</p>
          <p className="text-ink-700 text-sm mt-1">Be the first to predict something</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
